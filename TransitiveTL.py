...
parser.add_argument("--mode", help="mode of selecting categories", default='2')
parser.add_argument("--tMin", help="lower bound for target categories image numbers", default='20')
parser.add_argument("--tMax", help="upper bound for target categories image numbers", default='40')
parser.add_argument("--tNum", help="number of target categories", default='64')
parser.add_argument("--iMin", help="lower bound for intermediate categories image numbers", default='60')
parser.add_argument("--iMax", help="upper bound for intermediate categories image numbers", default='100')
parser.add_argument("--iNum", help="number of intermediate categories", default='32')
...
parser.add_argument("--testSize", help="share of test dataset in whole dataset", default='0.2')
#################################
...
earlyStoppingForBottleneck=EarlyStopping(monitor='val_loss', min_delta=0.01, patience = 2, verbose=0, mode='auto')
#################################

#################################
#get experiment id, this id will be increment automatically by 1
input_file = args.dir # which super category you want to train
Id_file = input_file + '_IdTL.txt'
Id = getExperimentId(Id_file)
#################################

#################################
#create report directory
report_dir_re = input_file +'_reportTL'
report_dir = input_file+ r'_reportTL/'
...
csv_columns = ['Id', 'Input Directory', 'Mode of selecting categories', 
               ...
               'Average images number of target', 'Average images number of intermediate', 'Increase', 'Increase Rate']
...
X_train, X_test, y_train, y_test = train_test_split(images, res, test_size=float(args.testSize), random_state=2)
#Data agumentation
...
history_ft_normal = model.compile(optimizer=SGD(lr=(learning_rate * 2.0), momentum=momen, decay = de), loss='categorical_crossentropy', metrics=['accuracy'])
#################################

#################################
print('Training (Phase 1)------------')
model.fit_generator(datagen.flow(X_train, y_train, batch_size=b_size), steps_per_epoch=len(X_train) / b_size, 
                    epochs=e_cpoch, verbose=1, validation_data=(X_test, y_test), callbacks=[earlyStoppingForBottleneck], shuffle=True)
#################################
...
average_I = len(X_train1) / number_of_classess1
average_T = len(X_train) / number_of_classess
#Data agumentation
...
for layer in base_model1.layers:
    layer.trainable = False
#training dense layer
history_ft_temp = model1.compile(optimizer=SGD(lr=(learning_rate * 2.0), momentum=momen, decay = de), loss='categorical_crossentropy', metrics=['accuracy'])
...
model1.fit_generator(datagen.flow(X_train1, y_train1, batch_size=b_size), steps_per_epoch=len(X_train1) / b_size, 
                    epochs=e_cpoch, verbose=1, validation_data=(X_test1, y_test1), callbacks=[earlyStoppingForBottleneck], shuffle=True)
...
for layer in base_model1.layers:
    layer.trainable = False
history_ft = model1.compile(optimizer=SGD(lr=(learning_rate * 2.0), momentum=momen, decay = de), loss='categorical_crossentropy', metrics=['accuracy'])
...
summarys = ["{:d}".format(Id), input_file, m, 
               ...
               "{:.4f}".format(average_T), "{:.4f}".format(average_I),
               "{:.4f}".format(accuracy_transitive - accuracy_normal), "{:.4f}".format( (accuracy_transitive - accuracy_normal) / accuracy_normal)]
