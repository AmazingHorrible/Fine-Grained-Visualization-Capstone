...
def plot_training(history, Id):
    if os.path.isdir("plots"):
        acc_path = r'plots/' + str(Id) + '_Accuracy.png'
        loss_path = r'plots/' + str(Id) + '_Loss.png'
    else:
        os.mkdir("plots")
        acc_path = r'plots/' + str(Id) + '_Accuracy.png'
        loss_path = r'plots/' + str(Id) + '_Loss.png'
...
earlyStopping=EarlyStopping(monitor='val_acc', min_delta=0.0001, patience = 2, verbose=0, mode='max')
...
history_ft = model.compile(optimizer='rmsprop', loss='categorical_crossentropy', metrics=['accuracy'])

print('Training ------------')
model.fit(X_train, y_train, batch_size=b_size, epochs=e_cpoch, verbose=1, validation_data=(X_test, y_test), shuffle=True)
